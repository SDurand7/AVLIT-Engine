...
void main() {
    ...
    if(l.type == AMBIENT_LIGHT) {
        outFragColor = vec4(texture(occlusion, textureCoord).x * l.color * texture(ambientColor, textureCoord).xyz, alpha);
    } else {
        ...
        outFragColor = vec4(shadow(vec4(position, 1.f), normal, lightV) * attenuation(l, lightV, position.xyz) * l.color 
                                * (computeDiffuse(lightV, position.xyz, normal) + computeSpecular(lightV, position.xyz, normal)), alpha);
    }    
}